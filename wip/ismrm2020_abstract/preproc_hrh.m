function S = preproc_hrh( reconDir )
...
dataDir     = fullfile( reconDir, 'data_hrh' );
maskDir     = fullfile( reconDir, 'mask' );
ktreconDir  = fullfile( reconDir, 'ktrecon_hrh' );
hrRange     = [105,180];
nFrameCine  = 25;
delta       = 150;
...
rltFileList       = dir( fullfile( dataDir, '*_rlt_hrh_ab.nii.gz' ) );
...
for iStk = 1:nStack
    ...
    % Identify Files
    S(iStk).desc          = strrep( rltFileList(iStk).name, '_rlt_hrh_ab.nii.gz', '' );
    ...
    S(iStk).rltMatFile    = fullfile( ktreconDir, sprintf( '%s_hrh_recon.mat', S(iStk).desc ) );
    S(iStk).rltParamFile  = fullfile( ktreconDir, sprintf( '%s_rlt_hrh_parameters.mat', S(iStk).desc ) );
    S(iStk).dcAbFile      = fullfile( dataDir, sprintf( '%s_dc_hrh_ab.nii.gz', S(iStk).desc ) );
