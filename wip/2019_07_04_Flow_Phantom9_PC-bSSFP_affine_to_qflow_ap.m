...
%% Step 4)
%% Transform stacks into same space
% FFD: ../reconDir/data_trans/register_bssfp.bash

%% Step 5)
%% Load stacks
reconDir = 'E:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_21_Flow_Phantom9\PC-bSSFP';
cd(reconDir);
cd data_affine_to_qflow

magFiles = dir('*_mag_affine.nii.gz');
...
disp('Getting gradient first moments ...');
...
M = load_nii('../mask/inside_pipes_mask2.nii.gz');
M = double(M.img);
...
save('../FP9_bSSFP_affine2qflow.mat','VEL','M');
...
templateNii = 'ax_mag_affine.nii.gz';
nii = load_nii(templateNii);
nii.hdr.dime.glmax = 30;
nii.hdr.dime.glmin = -30;

mkdir([num2str(numel(stackChoices)) '_phase_stacks_recon_sagCorr']);
cd([num2str(numel(stackChoices)) '_phase_stacks_recon_sagCorr']);

nii.img = VEL.rl;
save_nii(nii,'rl_vel_affine.nii.gz');
nii.img = VEL.ap;
save_nii(nii,'ap_vel_affine.nii.gz');
nii.img = -1 .* VEL.fh; %%% IMPORTANT FOR MRtrix - must flip z/fh direction
save_nii(nii,'fh_vel_affine.nii.gz');
