%% 2019_07_04 --- PC-bSSFP --- FlowPhantom_6 --- spherical flask
%
% NB: uses downsampled data - 128x128x120px to match the QFlow
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
reconDir = 'C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\PC-bSSFP';
cd(reconDir);
...
cd(reconDir);
cd data_128px
fileNames = dir('*_re_128px.nii.gz');

for ii = 1:numel(fileNames)
    currStr = erase(fileNames(ii).name, '_re_128px.nii.gz');
    
    re = load_untouch_nii( [currStr '_re_128px.nii.gz'] );
    im = load_untouch_nii( [currStr '_im_128px.nii.gz'] );
    ...
    save_untouch_nii(ph,[currStr '_ph_128px.nii.gz']);
    ...
    cd ../data_128px
    save_untouch_nii(ph_corr,[currStr '_ph_corr_128px.nii.gz']);
end

%% Step 4)
%% Register stacks into same space
% FFD: ../reconDir/data_128px_ffd/register_bssfp.bash
...
W = load_untouch_nii('../mask/static_water_mask_128px_ffd.nii.gz');
B = load_untouch_nii('../mask/background_mask_128px.nii.gz');
M = ~W.img & ~B.img;
...
implay_RR([ ...permute(QFLOW.VEL.sag,permOrder), ...
          ...
          M.*VEL.ap, M.*VEL.fh, M.*VEL.rl ...
           ],'jet',[-5,5]);
       
save('../FP8_bSSFP_128px.mat','VEL','M');
...
templateNii = 'ax_mag_128px_ffd.nii.gz';
nii = load_untouch_nii(templateNii);
nii.hdr.dime.glmax = 30;
nii.hdr.dime.glmin = -30;

nii.img = VEL.rl;
save_untouch_nii(nii,'rl_vel_128px_ffd.nii.gz');
nii.img = VEL.ap;
save_untouch_nii(nii,'ap_vel_128px_ffd.nii.gz');
nii.img = -1 .* VEL.fh; %%% IMPORTANT FOR MRtrix - must flip z/fh direction
save_untouch_nii(nii,'fh_vel_128px_ffd.nii.gz');
