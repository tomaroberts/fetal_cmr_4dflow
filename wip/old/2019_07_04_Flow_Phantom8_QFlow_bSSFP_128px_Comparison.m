% qflow
qDir = 'C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\QFlow\data';
cd(qDir);
...
venc = 30;
vq = double(venc * (PH./pi));

% % mask
% M = load_nii('../mask/inside_pipes_mask_trans_qflow2bssfp128px.nii.gz');
% M = single(M.img);
% 
% implay_RR([M.*vq(:,:,:,1), ...
%            M.*vq(:,:,:,2), ...
...
% bssfp - downsampled to 128x128
bDir = 'C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\PC-bSSFP\data_128px_ffd';
cd(bDir);

nii = load_nii('ap_vel_128px_ffd.nii.gz');
vb(:,:,:,1) = double(nii.img);
nii = load_nii('fh_vel_128px_ffd.nii.gz');
vb(:,:,:,2) = -1.*double(nii.img);
nii = load_nii('rl_vel_128px_ffd.nii.gz');
vb(:,:,:,3) = double(nii.img);


% masks
cd('C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\QFlow\mask');
P = load_nii('inside_pipes_mask_trans_qflow2bssfp128px.nii.gz');

cd('C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\PC-bSSFP\mask');
