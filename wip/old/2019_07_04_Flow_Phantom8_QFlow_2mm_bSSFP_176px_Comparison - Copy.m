% qflow - 2mm
qDir = 'C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\QFlow\data_2mm';
cd(qDir);

nii = load_nii('ap_vel_resample.nii.gz');
vq(:,:,:,1) = double(nii.img);
nii = load_nii('fh_vel_resample.nii.gz');
vq(:,:,:,2) = double(nii.img);
nii = load_nii('rl_vel_resample.nii.gz');
vq(:,:,:,3) = double(nii.img);


% bssfp
bDir = 'C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\PC-bSSFP\data_ffd';
cd(bDir);
...
P = load_nii('inside_pipes_mask_trans.nii.gz');

cd('C:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_04_Flow_Phantom8\PC-bSSFP\mask');
W = load_nii('static_water_mask_ffd.nii.gz');
% B = load_nii('background_mask.nii.gz');
B = load_nii('background_and_artefact_mask.nii.gz');
