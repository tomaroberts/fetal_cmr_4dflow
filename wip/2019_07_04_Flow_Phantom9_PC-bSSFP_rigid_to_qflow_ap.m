...
reconDir = 'E:\Users\tr17\Documents\Projects\PC_Fetal_CMR\Data\4D_Flow_Paper\2019_07_21_Flow_Phantom9\PC-bSSFP';
cd(reconDir);
cd data_rigid_to_qflow

magFiles = dir('*_mag_rigid.nii.gz');
...
save('../FP9_bSSFP_rigid2qflow.mat','VEL','M');
...
templateNii = 'ax_mag_rigid.nii.gz';
nii = load_nii(templateNii);
nii.hdr.dime.glmax = 30;
nii.hdr.dime.glmin = -30;

mkdir([num2str(numel(stackChoices)) '_phase_stacks_recon']);
cd([num2str(numel(stackChoices)) '_phase_stacks_recon']);

nii.img = VEL.rl;
save_nii(nii,'rl_vel_rigid.nii.gz');
nii.img = VEL.ap;
save_nii(nii,'ap_vel_rigid.nii.gz');
nii.img = -1 .* VEL.fh; %%% IMPORTANT FOR MRtrix - must flip z/fh direction
save_nii(nii,'fh_vel_rigid.nii.gz');
